function feature = nc_temporal_bins(T);
    ...
    % .01 to .1 Hz is "range of haemodynamic responses detectable with BOLD
    % fMRI. "low" is 0 to .005 (what is our bandpass at?) (Tohnka);
    ...
    for i=1:length(bucket_starts);
        ...
        bucket_start_freq = bucket_starts(i); % starting frequency of your bucket;
        bucket_end_freq = bucket_ends(i);   % end frequency of your bucket;
        ...
        freq_data = temp(1:floor(ntime/2)); % just take half the spectrum (because fft is symmetric);
